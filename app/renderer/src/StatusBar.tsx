...
interface StatusBarProps {
  ...
  mergeReady?: { ahead: number; onClick: () => void } | null
}
...
export function StatusBar({
  ...
  return (
    <div className="status-bar">
      ...
      {thinkingLevel && <span className="status-item thinking-level">{thinkingLevel}</span>}
      <span className={`status-item perm-mode ${permissionMode}`}>{permissionMode}</span>
      {mergeReady && mergeReady.ahead > 0 && (
        <button type="button" className="status-item merge-ready-chip" onClick={mergeReady.onClick}>
          ⎇ {mergeReady.ahead}↑ ready to merge
        </button>
      )}
      {contextLimit > 0 && (
